    ...
    selectedQuestions.forEach((q, index) => {
        const questionHTML = `
            <div style="margin: 20px; padding: 10px; border-radius: 10px;">
                ...
                <span id="targetQuarts${index}" style="font-weight: bold;">${q.targetQuarts}</span>
...
function generateValues() {
    ...
    values.forEach(value => {
        const valueHTML = `
            <td><div draggable="true" ondragstart="drag(event)" id="${value} gallons" style="background-color: #ffcccc; border: 2px solid #333; border-radius: 5px; padding: 5px; cursor: move;">${value} gallons</div></td>
        `;
...
function checkAnswer(index) {
    ...
    const targetQuarts = selectedQuestions[index].targetQuarts;

    if (!isNaN(first) && !isNaN(second)) {
        const calculatedQuarts = 4 * (first + second) + ' quarts';
        if (calculatedQuarts === targetQuarts) {
            if (!document.getElementById(`result${index}`).innerText) {
                document.getElementById('result').innerText = `Correct! The total number of quarts is ${calculatedQuarts}.☺️`;
